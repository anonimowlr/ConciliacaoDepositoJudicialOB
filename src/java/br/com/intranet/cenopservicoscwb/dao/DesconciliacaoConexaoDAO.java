 ...
 * @author PC_LENOVO
 */
public class DesconciliacaoConexaoDAO implements CrudDAO<DesconciliacaoOB> {
    ...
    public List<DesconciliacaoOB> buscar() throws ErroSistema {
        ...
        try {
            Connection con = ConnectionFactory.conectar("rejud_ob");
            String sql = "SELECT * FROM rejud_ob.tb_desconciliacao_ob_paj where ((NOME_TRATAMENTO = '' OR NOME_TRATAMENTO is null) AND (AVOCADO = '' OR AVOCADO IS NULL OR( AVOCADO = 'SIM' AND FUNCIONARIO_ATUAL =  '" + usuario.getChave() + "' ) ) AND(DATA_ENTRADA_BD IS NOT NULL) AND (SITUACAO = '' OR SITUACAO IS NULL OR SITUACAO = 'PRIORIZADO' ) AND  (DIAS_DESCONCILIADO <=30 AND DIAS_DESCONCILIADO>=20 )  and (DATA_PRIMEIRO_TRATAMENTO IS NULL )  OR ((SITUACAO = 'RETORNADO AGENCIA') AND (AVOCADO = '' OR AVOCADO IS NULL OR( AVOCADO = 'SIM' AND FUNCIONARIO_ATUAL =  '" + usuario.getChave() + "' ) )   AND  (DIAS_DESCONCILIADO <=30 AND DIAS_DESCONCILIADO>=20 ) AND (DATA_RETORNO_AGENCIA IS NOT NULL)  )) ORDER BY  DATA_ENTRADA_BD ASC,VALOR_DESCONCILIACAO DESC LIMIT 1";
