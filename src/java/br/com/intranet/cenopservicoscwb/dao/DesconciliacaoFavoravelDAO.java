 ...
 * @author PC_LENOVO
 */
public class DesconciliacaoFavoravelDAO implements CrudDAO<DesconciliacaoOB> {
    ...
    public List<DesconciliacaoOB> buscar() throws ErroSistema {
        ...
        try {
            Connection con = ConnectionFactory.conectar("rejud_ob");
            String sql = "SELECT * FROM rejud_ob.tb_desconciliacao_ob_paj where  SITUACAO = 'FAVORAVEL'  ";
         ...
         try {
           ...
           stmt.execute();
            ...
            stmt.execute();
             ...
             sql = "update rejud_ob.tb_desconciliacao_ob_paj  as t1 set SITUACAO = 'REGULARIZADO', NOME_TRATAMENTO = 'ANOTAÇÃO AUTOMÁTICA',DATA_SITUACAO = curdate() WHERE  exists (select * from rejud_ob.tb_tmp_concluidos as t2 where t1.NPJ=t2.NPJ and t1.CONTA_DEPOSITARIA=t2.CONTA_DEPOSITARIA and (t1.SITUACAO is null or t1.SITUACAO = ''))";
             stmt = con.prepareStatement(sql);
             stmt.execute();
            
             
             sql = "INSERT INTO rejud_ob.tb_serie_historica_desconciliacao_dj_paj(QUANTIDADE,VALOR,DATA,VALOR_ABSOLUTO) (select count(CODIGO) AS QUANTIDADE,sum((VALOR_DESCONCILIACAO) ) AS VALOR, curdate(),SUM(ABS(VALOR_DESCONCILIACAO)) AS VLR_ABSOLUTO from rejud_ob.tb_temporaria_desconciliacao_ob_paj)";
             stmt = con.prepareStatement(sql);
             stmt.execute();
            ...
            stmt.execute();
             ...
             sql = "UPDATE rejud_ob.tb_desconciliacao_ob_paj  as t1 set TRATADO_PRAZO = 'SIM' where DIAS_DESCONCILIADO  <=10 and DIAS_DESCONCILIADO>=0 AND (TRATADO_PRAZO IS NULL OR TRATADO_PRAZO = 'NAO') AND DATA_PRIMEIRO_TRATAMENTO IS NOT NULL AND SITUACAO = 'REGULARIZADO' AND NOME_TRATAMENTO = 'ANOTAÇÃO AUTOMÁTICA'";
             stmt = con.prepareStatement(sql);
             stmt.execute();
