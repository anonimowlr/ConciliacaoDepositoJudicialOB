 ...
 * @author PC_LENOVO
 */
public class DesconciliacaoNaoTratadaDAO implements CrudDAO<DesconciliacaoOB> {
    ...
    public List<DesconciliacaoOB> buscar() throws ErroSistema {
        try {
            Connection con = ConnectionFactory.conectar("rejud_ob");
            String sql = "SELECT * FROM tb_desconciliacao_ob_paj where (SITUACAO IS NULL OR SITUACAO= '') and(DATA_PRIMEIRO_TRATAMENTO IS NULL OR DATA_PRIMEIRO_TRATAMENTO = '') order by DIAS_DESCONCILIADO DESC";
    ...
    public Boolean avaliarParaSalvar(DesconciliacaoOB entidade) {
      ...
      GrupoTratamentoDAO tratamentoDAO = new GrupoTratamentoDAO();
        ...
        try {
            ...
            Logger.getLogger(DesconciliacaoOB.class.getName()).log(Level.SEVERE, null, ex);
        }
