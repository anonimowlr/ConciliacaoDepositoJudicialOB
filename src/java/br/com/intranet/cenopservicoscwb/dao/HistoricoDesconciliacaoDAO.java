 ...
 * @author jocimar
 */
public class HistoricoDesconciliacaoDAO implements  CrudDAO<HistoricoDesconciliacao>{
int qtdReg =  0;
    @Override
    ...
    public List<HistoricoDesconciliacao> buscar() throws ErroSistema {
       String hoje = null;
        try {
            hoje = Utils.getDataAtualFormatoMysql().toString();
        } catch (Exception ex) {
            Logger.getLogger(HistoricoDesconciliacaoDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
                
        try {
            Connection con = ConnectionFactory.conectar("rejud_ob");
            String sql = "SELECT * FROM tb_historico_desconciliacao_ob_paj WHERE DATA_SITUACAO  LIKE  '%'  '" + hoje + "' '%' ";
